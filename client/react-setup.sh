...
mkdir __mocks__ config src public \
    src/components src/layout src/pages src/test \
    src/layout/Header src/layout/Footer
# Stub files
touch \
    README.md .gitignore babel.config.js netlify.toml \
    ...
    public/index.html \
    src/components/index.js src/layout/index.js src/pages/index.js src/test/setupTests.js \
...
<html lang="en">
  <head>
      ...
      <meta name="description" content="My portfolio website, built with React!">
      <title>Calum McDougall Portfolio</title>
  </head>
  <body>
      <div id="root"></div>
  </body>
...
npx npm-add-script \
  ...
  --force
# Edit package.json using the npm package json
